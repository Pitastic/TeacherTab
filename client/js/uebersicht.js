...
function listStudents(results, option) {
	...
	var c, r, ul, old, row, omndl, ofspz, oschr, gesamt, len = results.rows.length;
	old = document.getElementById("listStudents").getElementsByTagName('ul')[0];
	ul = document.createElement('ul');
	for (var i=0; i<len; i++){
		row = results.rows.item(i);
		omndl = parseFloat(row.omndl);
        ofspz = JSON.parse(decodeURIComponent(row.ofspz)).Gesamt;
        oschr = parseFloat(row.oschr);
		gesamt = JSON.parse(decodeURIComponent(row.gesamt));
		...
		r.setAttribute('data-rowid', row.id);
			...
			c = document.createElement('div');
				...
				r.appendChild(c);
			if (gesamt.eingetragen == "-"){
                c = document.createElement('div');
                    c.className = "gesamt";
                    c.innerHTML = (gesamt.rechnerisch) ? "&#216; "+gesamt.rechnerisch.toPrecision(3) : "&#216; - . --";
                    r.appendChild(c);
			}else{
                c = document.createElement('div');
                    c.className = "gesamt eingetragen";
                    c.innerHTML = gesamt.eingetragen;
                    r.appendChild(c);
			}
...
function neueLeistung(thisElement){
    ...
    //var notenBezSel = document.getElementById('notenBezeichnung_Select');
...
// - - Textimport vieler Schülernamen
