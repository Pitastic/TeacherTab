$(document).ready(function() {
    ...
    pop.getElementsByClassName('button ABORT')[0].addEventListener('click', function(){
        if (window.confirm('Bist du sicher, dass du diese Leistung und alle eingetragenen Daten dazu unwiderruflich löschen möchtest ?')){dropLeistung()}
    });

	// List first View
	var column = sessionStorage.getItem('leistung_column');
	var id_Leistung = sessionStorage.getItem('leistung_id');
	leistungsDetails(column, id_Leistung);
});

// Listings
// =============================================================================
// >>>>>>>> Routing - Art der Eintragung filtern
function leistungsDetails(column, id_Leistung){
    // - - - Kopfdaten - - -
    db.transaction(function(transaction) {
        transaction.executeSql(
            "SELECT nName, "+column+" FROM "+klasse+" WHERE id=0", [], function(transaction, results) {
                // Leistungsobjekt bestimmter Spalte, bestimmter Zeitstempel erstellen
                var Leistung = JSON.parse(decodeURIComponent(results.rows.item(0)[column]))[id_Leistung];
                var katObj = JSON.parse(decodeURIComponent(results.rows.item(0).nName));
                var nArt = Leistung.Eintragung;
                sessionStorage.setItem('leistung_gewicht', Leistung.Gewichtung || 1);
                sessionStorage.setItem('Eintragung', nArt);
                leistungEdit.notenBezeichnung.value = Leistung.Bezeichnung;
                leistungEdit.notenDatum.value = Leistung.Datum;
                leistungEdit.notenArt.value = column;
				leistungEdit.rangeSlide.value = parseFloat(Leistung.Gewichtung) < 1 ? 0:parseFloat(Leistung.Gewichtung);
				leistungEdit.rangeWert.value = parseFloat(Leistung.Gewichtung) || 1;
                fspz_Bezeichnung();
                switch (nArt) {
                    case "Rohpunkte":
                        leistungsDetails_rohpunkte(column, Leistung, katObj);
                        break;
                    case "Noten":
                        leistungsDetails_noten(column, Leistung);
                        break;
                    case "Punkte":
                        leistungsDetails_punkte(column, Leistung);
                        break;
                }
            });
        return true;
...
// =============================================================================
// >>>>>>>> nach ganzen Noten
function leistungsDetails_noten(column, Leistung){
            ...
            "SELECT id, nName, vName, sex, "+column+" FROM "+klasse+" WHERE id !=0 ORDER BY nName", [], function(transaction,results){
                ...
                for (i=0; i<results.rows.length; i++){
                    ...
                    if (!Leistung || Leistung.Mitschreiber == "false" || !Leistung.Mitschreiber == "undefined" || !Leistung.Mitschreiber){
                        Leistung = {'Mitschreiber':'false', 'Note':'-', 'Kat1':'-', 'Kat2':'-', 'Kat3':'-', 'Kat4':'-',};
                    }
...
//--> Leistungen der Schüler speichern
    ...
    var liID, row, liKat, i2, keyVal, note, newObject = {};
    for (i=0;i<liAll.length;i++){
        ...
        newObject.Mitschreiber = row.getAttribute('data-mitschreiber');
        newObject.Gewichtung = parseFloat(sessionStorage.getItem('leistung_gewicht'));
        if (newObject.Mitschreiber == "true"){
            newObject.Note = note[0].innerHTML;
            newObject.Bezeichnung = Bezeichnung;
            if (bol_kat){
                newObject.Prozent = note[1].innerHTML;
                newObject.Verteilung = row.getAttribute('data-verteilung');
                liKat = row.getElementsByClassName('Kategorien');
                for (i2=0;i2<liKat.length;i2++){
                    keyVal = liKat[i2].getElementsByTagName('span')[0];
                    newObject[keyVal.getAttribute('data-name')] = parseFloat(keyVal.innerHTML);
                }
                ...
                newObject.Gesamt = parseFloat(keyVal.innerHTML);
            }    
        }
        // Object in Schüler Dict einfügen
...
function deleteTest(leistung){
    db.transaction(
            function(transaction){
            transaction.executeSql(
            'DELETE '+leistung+' FROM '+klasse+';', [], function(t, results){
                console.log(leistung+" wurde gelöscht !");
        });
    });
}
